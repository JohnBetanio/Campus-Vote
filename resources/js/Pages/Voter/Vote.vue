<template>
    <DashboardLayout
        page-title="Vote Now"
        page-subtitle="Cast your votes carefully. Once submitted, votes cannot be changed."
    >
        <div class="mx-auto flex max-w-6xl flex-col gap-6">
            <section
                v-if="activeElections.length > 1"
                class="vote-election-selector"
            >
            ...
            <header class="vote-header">
                ...
                <p class="vote-header-subtitle">
                    Cast your votes carefully. Once submitted, votes cannot be
                    changed.
                </p>
            </header>
            ...
            <form @submit.prevent="submitVote" class="vote-form">
                ...
                <div class="vote-positions-grid">
                    <section
                        ...
                        :class="{
                            complete:
                                selectedCount(position.id) ===
                                (position.max_votes || 1),
                        }"
                    >
                        <div class="position-card-header">
                            <h3 class="position-name">{{ position.name }}</h3>
                            <span class="position-max"
                                >Select up to
                                {{ position.max_votes || 1 }}</span
                            >
                        </div>
                        ...
                        <div class="candidates-list">
                            ...
                            >
                                <input
                                    ...
                                    @change="
                                        onCandidateChange(position, $event)
                                    "
                                />
...
function submitVote() {
    if (!canSubmit.value) {
        validationMessage.value =
            "Please select the required number of candidates for each position.";
        return;
    }
