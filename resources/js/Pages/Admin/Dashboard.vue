<template>
  ...
  >
    <template #actions>
      <Link :href="urls.admin.elections_create" class="btn btn-primary">+ Add Election</Link>
      <Link :href="urls.admin.elections_index" class="btn btn-secondary">Manage Elections</Link>
    </template>
    ...
    <div class="dashboard-content" style="max-width: 1280px; margin: 0 auto">
      ...
      <section class="kpi-grid" aria-label="Key metrics">
        ...
        >
          ...
          <div class="kpi-card__detail">
            {{ activeElectionsCount }} active • {{ Math.max(0, totalElections - activeElectionsCount) }} ended
          </div>
        </Link>
        ...
        >
          ...
          <div class="kpi-card__detail">
            {{ totalVotedCount }} {{ totalVotedCount === 1 ? 'has' : 'have' }} participated
          </div>
        </Link>
        ...
        >
          ...
          <div class="kpi-card__detail">
            {{ totalVotesCast }} {{ totalVotesCast === 1 ? 'ballot' : 'ballots' }} cast
          </div>
          <div class="kpi-card__progress" role="presentation">
            <div class="kpi-card__progress-fill" :style="{ width: Math.min(100, participationRate) + '%' }"></div>
          </div>
        </Link>
      ...
      <section class="card quick-actions-card" aria-labelledby="quick-actions-heading">
        <h2 id="quick-actions-heading" class="quick-actions-heading">Quick actions</h2>
        <div class="quick-actions-wrap">
          <Link :href="urls.admin.elections_create" class="quick-action-btn">Create election</Link>
          <Link :href="urls.admin.elections_index" class="quick-action-btn">Manage elections</Link>
          <Link :href="urls.admin.voters_index" class="quick-action-btn">Manage voters</Link>
          <Link :href="urls.admin.announcements_index" class="quick-action-btn">Announcements</Link>
        </div>
      </section>

      <!-- Active Elections -->
      <section v-if="activeElections.length > 0" class="card" aria-labelledby="active-elections-heading">
        ...
        >
          <div class="active-election-header">
            <div>
              <h3 class="active-election-title">{{ election.title }}</h3>
              <p class="active-election-meta">Created {{ formatDate(election.created_at) }}</p>
            </div>
            <Link :href="election.edit_url" class="btn btn-secondary btn-sm">Edit election</Link>
          </div>

          <div class="election-stats-grid">
            <div class="election-stat-box">
              <p class="election-stat-label">Votes cast</p>
              <p class="election-stat-value">{{ election.votes_count }}</p>
            </div>
            <div class="election-stat-box">
              <p class="election-stat-label">Participation</p>
              <p class="election-stat-value">{{ election.participation_rate }}%</p>
            </div>
            <div class="election-stat-box">
              <p class="election-stat-label">Positions</p>
              <p class="election-stat-value">{{ election.positions_count }}</p>
            </div>
          </div>
          ...
          <template v-if="getElectionResults(election.id)">
            <div
              v-for="(candidates, positionName) in getElectionResults(election.id)"
              :key="positionName"
              class="result-chart"
            >
              <h4 class="result-position-title">{{ positionName }}</h4>
              <div v-for="(candidate, idx) in candidates" :key="idx" class="result-candidate-row">
                ...
                <div class="result-progress-track">
                  ...
                  >
                    <span v-if="candidate.percentage > 10" class="result-progress-text">
